...
class _CameraHomePageState extends State<CameraHomePage>
  ...
  Future<void> _start() async {
    ...
    // Open the camera as soon as the camera list is available. Database and
    // photo-history loading must never delay the first camera frame.
    try {
    ...
    // These are independent of the camera and can finish in the background.
    await Future.wait([_loadDatabase(), _loadPhotoHistory()]);
  ...
  Future<void> _initializeCamera() async {
    ...
    // There must only ever be one CameraController holding the camera
    // hardware. Keeping the old controller alive while opening another one
    // can make Android camera HALs stutter, freeze, or return a blank texture.
    ...
    try {
      ...
      // Camera preview is now live. Apply optional controls after the first
      // frame so they never block camera initialization.
      _applyCameraSettings(controller, token);
    } catch (e) {
      try {
        await controller.dispose();
      } catch (_) {}
  ...
  Future<void> _applyCameraSettings(CameraController controller, int token) async {
    ...
    try {
      ...
      if (!mounted || token != _cameraInitToken || !identical(_camera, controller)) return;
      _minZoom = minZoom;
      _maxZoom = maxZoom;
  ...
  Future<void> _handleCameraPaused() async {
    ++_cameraInitToken;
    _cameraBusy = false;
    _switchingCamera = false;
  ...
  Future<void> _handleCameraResumed() async {
    if (!mounted || cameras.isEmpty || _camera != null || _cameraBusy) return;

    // Android may need a short moment to recreate its Surface after screen-on.
    // One short wait is preferable to repeated initialize/retry loops.
    await Future<void>.delayed(const Duration(milliseconds: 120));
    if (!mounted || _camera != null || _cameraBusy) return;
  ...
  Future<void> _saveToGallery(Uint8List bytes, String filename) async {
    ...
    try {
      ...
      // Penyimpanan lokal tetap dipertahankan bila MediaStore gagal.
    }
  }
  ...
  Future<void> _takePhoto() async {
    ...
    try {
      // Ambil foto SECEPATNYA. Jangan menunggu proses stabilizer, resize,
      // encoding, Gallery, atau history sebelum shutter dijalankan.
      final photo = await camera.takePicture();

      // Shutter sudah selesai. Lepaskan indikator pengambilan sekarang agar
      // kamera kembali responsif sementara proses penyimpanan berjalan.
      _capturing = false;
      if (mounted) setState(() {});

      // Semua pekerjaan berat dilakukan setelah foto berhasil diambil.
      try {
        ...
        final targetRatio = _ratio == CameraRatio.full
        ...
        if (mounted) {
          _message('Foto tersimpan ke Gallery > Dokumentasi Barang Pecah. Status: SUDAH DIFOTO.');
        }
      } catch (e) {
        if (mounted) _message('Foto sudah diambil, tetapi proses penyimpanan gagal: $e');
      }
    } catch (e) {
      _capturing = false;
      if (mounted) setState(() {});
  ...
  Future<void> _switchCamera() async {
    if (cameras.length < 2 || _cameraBusy || _switchingCamera || !mounted) return;
    ...
    // Use a single controller at a time. This is slightly more conservative
    // than overlapping controllers, but prevents Android camera HAL freezes.
    await _initializeCamera();
  }
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: CameraRatio.values.map((r) => ChoiceChip(
                      ...
                      selected: _ratio == r,
                      onSelected: (_) { if (_ratio == r) return; setState(() => _ratio = r); setSheet(() {}); },
                    )).toList(),
                  ),
                  const SizedBox(height: 18),
                  ...
                  SwitchListTile(
                    ...
                    subtitle: const Text('Kurangi getaran saat mengambil foto'),
                    value: _stabilizerOn,
                    onChanged: (v) { setState(() => _stabilizerOn = v); setSheet(() {}); },
                  ),
                  const Text('Exposure', style: TextStyle(fontWeight: FontWeight.bold)),
                  Row(children: [
                    const Icon(Icons.exposure_minus_1),
                    Expanded(child: Slider(value: _exposure, min: -2, max: 2, divisions: 16, label: _exposure.toStringAsFixed(1), onChanged: (v) async { setState(() => _exposure = v); setSheet(() {}); try { await _camera?.setExposureOffset(v); } catch (_) {} })),
  ...
  Widget _cameraPreview() {
    ...
    // FULL tetap memenuhi layar. Untuk rasio lain, bidang preview benar-benar
    // berubah mengikuti rasio yang dipilih. CameraPreview hanya di-crop oleh
    // viewport; zoom optik kamera TIDAK diubah saat mengganti rasio, sehingga
    // gambar tidak menjadi cembung/terlalu dekat.
      ...
      child: Scaffold(
        ...
        body: Stack(
          ...
          children: [
            ...
            SafeArea(
              child: Align(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: Colors.black.withValues(alpha: .42), borderRadius: BorderRadius.circular(22)),
                  child: Row(children: [
            ...
            // Zoom lives in its own higher layer so it can never be covered
            // by the "Nomor barang (opsional)" input or the shutter bar.
                ...
                child: Container(
                  ...
                  child: Text(
                    _namaBarang.isNotEmpty
                        ? '${_numberController.text} • $_namaBarang'
                        : 'Nomor belum ditemukan',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      color: _namaBarang.isNotEmpty ? Colors.white : Colors.amber,
            ...
            Positioned(
              ...
              child: Column(children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: Colors.black.withValues(alpha: .52), borderRadius: BorderRadius.circular(18)),
                  child: Row(children: [
                    Expanded(child: TextField(controller: _numberController, keyboardType: TextInputType.number, inputFormatters: [FilteringTextInputFormatter.digitsOnly], onChanged: (v) => setState(() => _namaBarang = _findBarang(v) ?? ''), style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.bold), decoration: InputDecoration(hintText: _barang.isEmpty ? 'Nomor barang (opsional)' : 'Nomor barang', hintStyle: const TextStyle(color: Colors.white54), prefixIcon: const Icon(Icons.tag, color: Colors.white70), filled: true, fillColor: const Color(0xAA202124), border: const OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(14)), borderSide: BorderSide.none), contentPadding: const EdgeInsets.symmetric(vertical: 9)))),
...
class _PhotoEditorPageState extends State<PhotoEditorPage> {
  ...
  Future<void> _saveEdited() async {
    ...
    try {
      ...
      if (Platform.isAndroid) {
        try {
          await const MethodChannel('dokumentasi_barang_pecah/media_store').invokeMethod('saveImage', <String, dynamic>{
            'bytes': bytes,
            'displayName': target.path.split(Platform.pathSeparator).last,
            'relativePath': 'Pictures/Dokumentasi Barang Pecah',
          });
        } on PlatformException {
          // Salinan lokal tetap tersedia.
        }
      }
      if (mounted) {
        _message('Foto hasil edit berhasil disimpan ke Gallery.');
