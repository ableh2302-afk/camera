...
class _CameraHomePageState extends State<CameraHomePage>
  ...
  String get _folderPath => _photoRoot?.path ?? 'Penyimpanan aplikasi/Dokumentasi Barang Pecah';
  ...
  Future<void> _start() async {
    // Jangan menunggu izin storage/Excel/foto sebelum kamera tampil.
    final cameraPermission = await Permission.camera.request();
    if (!cameraPermission.isGranted) {
      if (mounted) {
        setState(() {
          _cameraError = 'Izin kamera belum diberikan.';
          _initializing = false;
        });
      }
      return;
    ...
    // Jangan tahan seluruh UI sampai kamera selesai initialize.
    // Kontrol kamera tetap tampil; preview akan aktif segera setelah siap.
    if (mounted) setState(() => _initializing = false);
    await _initializeCamera();

    // Data non-kamera dimuat terpisah dan tidak pernah me-restart kamera.
    await Future.wait([_loadDatabase(), _refreshPhotos()]);
  }
  ...
  Future<void> _initializeCamera() async {
    ...
    try {
      // Jangan mematikan preview lama sebelum controller baru benar-benar siap.
      ...
      await controller?.dispose();
      if (mounted && token == _cameraInitToken) {
        setState(() => _cameraError = 'Kamera gagal dibuka. Tap COBA LAGI.');
      }
    } finally {
      _cameraBusy = false;
    }
  ...
  Future<Directory> _photoDirectory() async {
    if (_photoRoot != null) {
      if (!await _photoRoot!.exists()) {
        await _photoRoot!.create(recursive: true);
      }
      return _photoRoot!;
    }

    // Android modern/scoped storage: jangan menulis langsung ke
    // /storage/emulated/0 karena File.copy() dapat ditolak dengan
    // Operation not permitted. Gunakan external app storage yang
    // memang diberikan kepada aplikasi.
    final base = await getExternalStorageDirectory();
    if (base == null) {
      throw FileSystemException('Folder penyimpanan aplikasi tidak tersedia');
    }
  ...
  Future<void> _takePhoto() async {
    ...
    try {
      ...
      await target.writeAsBytes(await File(photo.path).readAsBytes(), flush: true);
  ...
  Future<void> _switchCamera() async {
    ...
    // camera plugin pada sebagian perangkat tidak mengizinkan dua
    // controller memakai hardware kamera bersamaan. Lepaskan controller
    // lama terlebih dahulu agar kamera depan/belakang tidak berhenti hitam.
    final oldCamera = _camera;
    try {
      // Lepaskan hardware kamera lama sebelum membuka kamera berikutnya.
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: CameraRatio.values.map((r) => ChoiceChip(
                      ...
                      selected: _ratio == r,
                      onSelected: (_) async { setState(() => _ratio = r); setSheet(() {}); await _savePreferences(); },
                    )).toList(),
                  ),
                  const SizedBox(height: 18),
      ...
      if (_cameraError != null) {
        return Center(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Icon(Icons.no_photography_outlined, color: Colors.white54, size: 58),
              const SizedBox(height: 12),
              Text(_cameraError!, style: const TextStyle(color: Colors.white70)),
              const SizedBox(height: 12),
              FilledButton.icon(
                onPressed: _cameraBusy ? null : _initializeCamera,
...
class _PhotoEditorPageState extends State<PhotoEditorPage> {
  ...
  void _drawCircleOutline(img.Image image, {required int cx, required int cy, required int radius, required img.Color color, required int thickness}) {
    final outer = radius + (thickness / 2).ceil();
    final inner = math.max(0, radius - (thickness / 2).floor());
    final left = math.max(0, cx - outer);
    final right = math.min(image.width - 1, cx + outer);
    final top = math.max(0, cy - outer);
    final bottom = math.min(image.height - 1, cy + outer);
    final r2Outer = outer * outer;
    final r2Inner = inner * inner;
  ...
  void _drawMark(img.Image image, _EditorMark mark) {
    ...
    switch (mark.mode) {
      case AnnotationMode.circle:
        ...
        _drawCircleOutline(image, cx: cx, cy: cy, radius: math.max(8, radius), color: color, thickness: thickness);
        break;
      ...
      case AnnotationMode.number:
        _drawCircleOutline(image, cx: sx, cy: sy, radius: math.max(22, image.width ~/ 70), color: color, thickness: thickness);
