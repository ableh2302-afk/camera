...
class _CameraHomePageState extends State<CameraHomePage>
  ...
  bool _stabilizerOn = false;
  static const MethodChannel _mediaStore = MethodChannel('dokumentasi_barang_pecah/media_store');
  ...
  static const _folderPath = 'Pictures/Dokumentasi Barang Pecah';
  ...
  Future<void> _start() async {
    ...
    // Kamera dihidupkan lebih dulu supaya input nomor dan import Excel
    // tidak pernah menghambat standby kamera.
  ...
  Future<void> _requestPermissions() async {
    ...
    if (Platform.isAndroid && Platform.version.startsWith('Android')) {
      // Tidak meminta MANAGE_EXTERNAL_STORAGE. Foto publik disimpan via MediaStore.
    }
  }
  ...
  Future<void> _loadPreferences() async {
    ...
    _stabilizerOn = p.getBool('camera_stabilizer') ?? false;
  ...
  Future<void> _savePreferences() async {
    ...
    await p.setBool('camera_stabilizer', _stabilizerOn);
  }
  ...
  Future<void> _initializeCamera() async {
    if (cameras.isEmpty || _cameraBusy || !mounted) return;
    _cameraBusy = true;
    final token = ++_cameraInitToken;
    final selected = cameras[_cameraIndex.clamp(0, cameras.length - 1)];
    final controller = CameraController(
      selected, _resolution, enableAudio: false, imageFormatGroup: ImageFormatGroup.jpeg,
    );
    try {
      await controller.initialize();
      try { await controller.setFlashMode(_flashOn ? FlashMode.torch : FlashMode.off); } catch (_) {}
      ...
      if (!mounted || token != _cameraInitToken) { await controller.dispose(); return; }
      final previous = _camera;
      setState(() { _camera = controller; _switchingCamera = false; });
      await previous?.dispose();
    } catch (e) {
      await controller.dispose();
      if (mounted) { setState(() => _switchingCamera = false); _message('Kamera gagal dibuka: $e'); }
  ...
  Future<Directory> _photoDirectory() async {
    ...
    final dir = Directory('${base!.path}/Dokumentasi Barang Pecah');
  ...
  Future<File> _uniqueFile(Directory dir, String baseName) async {
    ...
    var n = 1;
    while (await f.exists()) { f = File('${dir.path}/${baseName}_$n.jpg'); n++; }
    return f;
  }

  Future<void> _saveToGallery(Uint8List bytes, String filename) async {
    if (!Platform.isAndroid) return;
    await _mediaStore.invokeMethod('saveImage', <String, dynamic>{
      'bytes': bytes,
      'displayName': filename,
      'relativePath': 'Pictures/Dokumentasi Barang Pecah',
    });
  }
  ...
  Future<void> _refreshPhotos() async {
    try {
      ...
      final files = dir.listSync().whereType<File>().where((f) {
        final p = f.path.toLowerCase(); return p.endsWith('.jpg') || p.endsWith('.jpeg') || p.endsWith('.png');
      }).toList();
      files.sort((a,b) => b.statSync().modified.compareTo(a.statSync().modified));
      if (mounted) setState(() { _photos = files; _latestPhoto = files.isEmpty ? null : files.first.path; });
    } catch (_) {}
  }
  ...
  Future<void> _takePhoto() async {
    ...
    if (camera == null || !camera.value.isInitialized) { _message('Kamera belum siap.'); return; }
    if (_timerSeconds > 0) { await _countdown(); if (!mounted) return; }
    setState(() => _saving = true);
    try {
      if (_stabilizerOn) await Future.delayed(const Duration(milliseconds: 350));
      ...
      final targetRatio = _ratio == CameraRatio.full ? screen.width / screen.height : _frameRatio;
      final currentRatio = decoded.width / decoded.height;
      if ((currentRatio-targetRatio).abs() > .015) {
        var cropW = decoded.width; var cropH = decoded.height;
        if (currentRatio > targetRatio) cropW = (decoded.height * targetRatio).round();
        else cropH = (decoded.width / targetRatio).round();
        cropW = cropW.clamp(1, decoded.width).toInt(); cropH = cropH.clamp(1, decoded.height).toInt();
        final x = ((decoded.width-cropW)/2).round().clamp(0, decoded.width-cropW).toInt();
        final y = ((decoded.height-cropH)/2).round().clamp(0, decoded.height-cropH).toInt();
        decoded = img.copyCrop(decoded, x:x, y:y, width:cropW, height:cropH);
      }
      final bytes = Uint8List.fromList(img.encodeJpg(decoded, quality: 95));
      final dir = await _photoDirectory();
      final entered = _numberController.text.trim();
      final matched = _findBarang(entered);
      String baseName; String? historyKey;
      if (matched != null) { baseName='${_cleanName(entered)}_${_cleanName(matched)}'; historyKey=_normalizeNumber(entered); }
      else if (entered.isNotEmpty) { baseName='${_cleanName(entered)}_FOTO'; historyKey=_normalizeNumber(entered); }
      else { baseName='FOTO_${DateTime.now().millisecondsSinceEpoch}'; }
      final target = await _uniqueFile(dir, baseName);
      await target.writeAsBytes(bytes, flush:true);
      await _saveToGallery(bytes, target.path.split(Platform.pathSeparator).last);
      if (historyKey != null) { _photoHistory.putIfAbsent(historyKey, ()=> <String>[]).add(target.path); await _savePhotoHistory(); }
      await _refreshPhotos();
      if (mounted) { setState(() => _saving=false); _message('Foto tersimpan ke Galeri > Dokumentasi Barang Pecah.'); }
    } catch (e) {
  ...
  Future<void> _toggleStabilizer() async {
    setState(() => _stabilizerOn = !_stabilizerOn);
    await _savePreferences();
    _message(_stabilizerOn ? 'Stabilizer aktif.' : 'Stabilizer dimatikan.');
  }
  ...
  Widget _cameraPreview() {
    ...
    if (camera == null || !camera.value.isInitialized) {
      return Container(color: Colors.black, alignment: Alignment.center, child: (_initializing || _switchingCamera) ? const CircularProgressIndicator(color: Colors.white) : const Text('KAMERA TIDAK SIAP', style: TextStyle(color: Colors.white54)));
    }
      ...
      child: Center(
        child: AspectRatio(
          ...
          child: ClipRect(
            child: FittedBox(
              fit: BoxFit.cover,
              child: SizedBox(width: ps.height, height: ps.width, child: CameraPreview(camera)),
            ),
          ),
        ),
      ),
  ...
  Widget _gridOverlay() => _gridOn ? IgnorePointer(child: CustomPaint(painter: GridPainter(), size: Size.infinite)) : const SizedBox.shrink();

  Widget _bottomBar() {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 8),
      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 5),
      decoration: BoxDecoration(color: Colors.black.withValues(alpha: .55), borderRadius: BorderRadius.circular(22)),
      child: SafeArea(
        top: false,
        child: Row(children: [
          ...
          Expanded(flex: 2, child: GestureDetector(onTap: _saving ? null : _takePhoto, child: Center(child: Container(width: 72,height:72,decoration: BoxDecoration(shape:BoxShape.circle,border:Border.all(color:Colors.white,width:5)),child:Container(margin:const EdgeInsets.all(5),decoration:const BoxDecoration(color:Colors.white,shape:BoxShape.circle))))),
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      value: const SystemUiOverlayStyle(statusBarColor: Colors.transparent, systemNavigationBarColor: Colors.transparent, statusBarIconBrightness: Brightness.light, systemNavigationBarIconBrightness: Brightness.light),
      child: Scaffold(
        backgroundColor: Colors.black, resizeToAvoidBottomInset: false,
        body: Stack(fit: StackFit.expand, children: [
          _cameraPreview(),
          _gridOverlay(),
          SafeArea(child: Align(alignment: Alignment.topCenter, child: Container(margin: const EdgeInsets.fromLTRB(7,4,7,0), padding: const EdgeInsets.symmetric(horizontal:5,vertical:2), decoration: BoxDecoration(color:Colors.black.withValues(alpha:.42),borderRadius:BorderRadius.circular(22)), child: Row(children: [
            _topButton(_flashOn ? Icons.flash_on : Icons.flash_off, 'FLASH', _toggleFlash, active:_flashOn),
            _topButton(_stabilizerOn ? Icons.vibration : Icons.pan_tool_outlined, 'STABIL', _toggleStabilizer, active:_stabilizerOn),
          ...
          Positioned(left:10,right:10,bottom:124,child: Column(children: [
            if (_barang.isNotEmpty || _numberController.text.isNotEmpty) Container(padding:const EdgeInsets.symmetric(horizontal:12,vertical:7),decoration:BoxDecoration(color:Colors.black.withValues(alpha:.5),borderRadius:BorderRadius.circular(14)),child:Text(_namaBarang.isNotEmpty ? '${_numberController.text} • $_namaBarang' : 'Nomor belum ditemukan',textAlign:TextAlign.center,style:TextStyle(color:_namaBarang.isNotEmpty?Colors.white:Colors.amber,fontWeight:FontWeight.bold))),
            if (_maxZoom > _minZoom) Row(children:[const Icon(Icons.zoom_out,color:Colors.white,size:18),Expanded(child:Slider(value:_zoom,min:_minZoom,max:_maxZoom,onChanged:(v)async{setState(()=>_zoom=v);try{await _camera?.setZoomLevel(v);}catch(_){}})),const Icon(Icons.zoom_in,color:Colors.white,size:18)]),
          ])),
          Positioned(left:8,right:8,bottom:7,child:Column(children:[
            Container(padding:const EdgeInsets.symmetric(horizontal:7,vertical:3),decoration:BoxDecoration(color:Colors.black.withValues(alpha:.52),borderRadius:BorderRadius.circular(18)),child:Row(children:[Expanded(child:TextField(controller:_numberController,keyboardType:TextInputType.number,inputFormatters:[FilteringTextInputFormatter.digitsOnly],onChanged:(v)=>setState(()=>_namaBarang=_findBarang(v)??''),style:const TextStyle(color:Colors.white,fontSize:17,fontWeight:FontWeight.bold),decoration:InputDecoration(hintText:_barang.isEmpty?'Nomor barang (opsional)':'Nomor barang',hintStyle:const TextStyle(color:Colors.white54),prefixIcon:const Icon(Icons.tag,color:Colors.white70),filled:true,fillColor:const Color(0xAA202124),border:OutlineInputBorder(borderRadius:BorderRadius.all(Radius.circular(14)),borderSide:BorderSide.none),contentPadding:const EdgeInsets.symmetric(vertical:9)))),IconButton(onPressed:_resetDatabase,color:Colors.white70,icon:const Icon(Icons.delete_sweep_outlined))])),
            const SizedBox(height:3), _bottomBar(),
          ])),
          if (_saving) const Positioned.fill(child: IgnorePointer(child: ColoredBox(color:Colors.black26,child:Center(child:SizedBox(width:40,height:40,child:CircularProgressIndicator(color:Colors.white))))),
        ]),
      ),
    );
  }
...
class PhotoStatusPage extends StatelessWidget {
  ...
  String _normalize(String value) {
    final v = value.trim();
    if (v.isEmpty) return '';
    final n = v.replaceFirst(RegExp(r'^0+'), '');
  ...
  Widget build(BuildContext context) {
    ...
    final done = keys.where((k) => (history[_normalize(k)] ?? const <String>[]).isNotEmpty).length;
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Expanded(
          child: keys.isEmpty
              ? const Center(child: Text('Import Excel terlebih dahulu.'))
              : ListView.separated(
                  ...
                  itemBuilder: (_, i) {
                    final no = keys[i];
                    final count = (history[_normalize(no)] ?? const <String>[]).length;
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        trailing: Text(
                          doneItem ? 'SUDAH DIFOTO\n$count FOTO' : 'BELUM DIFOTO',
                          textAlign: TextAlign.right,
                          style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: doneItem ? Colors.green : Colors.orange),
                        ),
                      ),
                    );
                  },
  ...
  void _drawMark(img.Image image, _EditorMark mark) {
    ...
    switch (mark.mode) {
      case AnnotationMode.circle:
        ...
        _drawCircleOutline(image, cx: cx, cy: cy, radius: math.max(8, radius).toInt(), color: color, thickness: thickness);
        break;
      ...
      case AnnotationMode.number:
        _drawCircleOutline(image, cx: sx, cy: sy, radius: math.max(22, image.width ~/ 70).toInt(), color: color, thickness: thickness);
