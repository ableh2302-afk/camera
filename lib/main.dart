...
class _CameraHomePageState extends State<CameraHomePage>
  ...
  Future<void> _start() async {
    // Startup dibuat tahan-crash: jangan meminta akses storage khusus Android
    // dan jangan melakukan proses Excel/galeri sebelum UI utama tampil.
    try {
      final status = await Permission.camera.request();
      if (!status.isGranted) {
        if (mounted) {
          setState(() => _initializing = false);
          _message('Izin kamera diperlukan. Silakan aktifkan izin Kamera.');
        }
        return;
      }
    } catch (_) {
      // Jika permission_handler gagal di perangkat tertentu, lanjutkan ke
      // inisialisasi kamera; error kamera akan ditangani secara aman.
    }

    // Preferensi tidak boleh menggagalkan startup.
    try { await _loadPreferences(); } catch (_) {}

    // Cari kamera setelah Flutter UI sudah berjalan, bukan di main().
    try {
    ...
    // Data Excel dan foto dimuat setelah kamera tampil.
    try { await _loadDatabase(); } catch (_) {}
    try { await _refreshPhotos(); } catch (_) {}
  }
