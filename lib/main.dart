...
class _CameraHomePageState extends State<CameraHomePage>
  ...
  Future<void> _initializeCamera() async {
    ...
    try {
      // The important part for responsiveness: wait only for the camera
      // hardware to initialize. Do NOT wait for zoom/exposure/flash before
      ...
      // Publish the new texture immediately after initialize(). This keeps
      // camera switching responsive and avoids a blank preview.
      setState(() {
        _camera = controller;
      ...
      // Dispose the old controller after the new texture has had a frame to
      // attach. A short delay is enough; do not keep two active controllers
      // around for hundreds of milliseconds.
      ...
      // Apply optional camera settings after the preview is already visible.
      // These operations must never block the first camera frame.
      try {
        if (_flashOn) {
  ...
  Future<void> _handleCameraPaused() async {
    // Android may invalidate the camera texture while the activity is not
    // visible. Release it cleanly so the next resume gets a fresh texture.
    ++_cameraInitToken;
  ...
  Future<void> _handleCameraResumed() async {
    if (_camera != null || _cameraBusy || cameras.isEmpty || !mounted) return;
    ...
    // Give Android a moment to finish recreating the Activity/Surface before
    // requesting the camera again. This avoids the transient "camera not
    // ready" state after the screen is turned back on.
  ...
  Future<void> _takePhoto() async {
    ...
    try {
      if (_stabilizerOn) await Future.delayed(const Duration(milliseconds: 250));
  ...
  Future<void> _switchCamera() async {
    // Keep the current preview visible while the next camera initializes.
    // Do not blank the UI and do not wait for zoom/exposure configuration.
    if (cameras.length < 2 || _cameraBusy || _switchingCamera || !mounted) return;
    setState(() {
      _cameraIndex = (_cameraIndex + 1) % cameras.length;
      _switchingCamera = true;
    });
    // Fire the initialization without blocking the button/UI.
    await _initializeCamera();
  }
